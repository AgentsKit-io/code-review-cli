...
export const computeSourceHash = (root, sources) => {
  ...
  for (const source of [...sources].sort()) {
    hash.update(source).update('\0').update(readFileSync(join(root, source))).update('\0')
  }
  return `sha256:${hash.digest('hex')}`
}

const imageReferences = (readme) => [
  ...readme.matchAll(/!\[([^\]]*)\]\(([^)]+)\)/g),
...
export const auditReadme = (root, config, now = new Date()) => {
  const failures = []
  if (config.schemaVersion !== 1 || config.standardId !== 'agentskit-readme-standard-v1' || config.status !== 'approved') {
    failures.push('README Standard v1 approval metadata is invalid')
  }
  ...
  for (const surface of config.surfaces) {
    ...
    for (const [dimension, markers] of Object.entries(surface.dimensions)) {
      for (const marker of markers) if (!readme.includes(marker)) failures.push(`${surface.id}: ${dimension} evidence missing: ${marker}`)
    }
    ...
    const images = imageReferences(readme)
    ...
    const expected = computeSourceHash(root, surface.freshness.sources)
    if (expected !== surface.freshness.sourceHash) failures.push(`${surface.id}: sourceHash is stale`)
  }
